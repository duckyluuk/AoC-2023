...
# Part 2: 274 bytes
print(sum(len(q:=[int(m[x:k])for m in I[max(0,i-1):i+2]for x in range(max(0,j-3),j+2) for k in range(max(x,j),x+4)if((k>=len(m))or 1-m[k].isdigit())*m[x:k].isdigit()*((x<1)or 1-m[x-1].isdigit())])==2 and q[0]*q[1]for i,l in enumerate(I)for j in range(len(l)-1)if l[j]=="*"))
